...
public class PatientInfoRest {
	...
	@POST
	@Path("/create")
	public Response createPatientInfo(final PatientInfo patientInfo) {
		String id = pi.createPatientInfo(patientInfo);
		patientInfo.setId(id);
		ResponseBuilder rb = Response.ok().entity(patientInfo);
		return rb.build();
	}
	...
	public Response findPatientInfo(@PathParam("id") final String id) {
		PatientInfo result = pi.findPatientInfo(id);
	...
	public Response searchPatientInfo(@QueryParam("fName") final String firstName,
		...
		List<PatientInfo> searchResults = pi.searchPatientInfo(firstName, lastName);
